module YmLikes::Likeable
  ...
  def self.included(base)
    base.has_many :likings, :as => :resource, :class_name => 'YmLikes::Like', :dependent => :destroy
    ...
    base.scope :liked_by, lambda{|user| base.joins(:likings).where(:likes => {:user_id => user.id})}
  end
